...
FloorPlanView SHALL render each `FloorPlanRoom` as an SVG rounded rectangle (`<rect rx="4">`), positioned by grid coordinates × cellSize. Each room SHALL display its translated name (`tRoomName`) centered as SVG text.
...
Unassigned rooms SHALL have dark fill (`#1a1f2e`) with gray dashed border. Assigned rooms SHALL have slightly lighter fill with a solid border in the circuit's assigned color.
