...
- panel: 4 slots / 50A mainBreaker
- budget: $100, survivalTime: 8s
- bonusCondition: { type: 'under-budget-ratio', ratio: 0.85 }

#### Scenario: L06 格式正確
- **WHEN** 讀取 LEVELS[5]
- **THEN** 'rooms' in level === true，且包含 2 個 room（客廳、廚房），panel.maxSlots=4, panel.mainBreakerRating=50
...
- panel: 6 slots / 75A mainBreaker
- budget: $250, survivalTime: 15s
- bonusCondition: { type: 'under-budget-ratio', ratio: 0.85 }
...
LEVELS[11] SHALL 為 FreeCircuitLevel + phaseMode: 'manual'，定義如下：
- name: 'L12 相位平衡進階'
- description: '更多電器、手動相位切換。你來決定哪條迴路走 R 相、哪條走 T 相。'
- rooms: 廚房 [快煮壺, 微波爐, 烤箱]、客廳 [電暖器, 吹風機]、儲藏室 [冰箱]、冷氣間 [冷氣(220V)]
- panel: 6 slots / 75A mainBreaker
- budget: $280, survivalTime: 15s
- phaseMode: 'manual'
- bonusCondition: { type: 'time-margin', margin: 3 }

#### Scenario: L12 manual phaseMode
- **WHEN** 讀取 LEVELS[11]
- **THEN** phaseMode === 'manual'，rooms 包含多種高功率 110V 電器
...
- panel: 8 slots / 75A mainBreaker
- budget: $400, survivalTime: 15s
